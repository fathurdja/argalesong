...
<div class="bg-white p-4 rounded-lg shadow-lg mt-20 ml-9">
    ...
    <div class="flex items-center mb-4 md:w-max-screen overflow-x-auto">
        ...
        <table class="min-w-full table-auto">
            <thead class="bg-gray-50">
                <tr>
                    ...
                    <th class="px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider">Saldo Piutang</th>
                    <th class="px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider hidden md:table-cell">Kode</th>
                    <th class="px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider hidden md:table-cell">Jatuh Tempo</th>
                    <th class="px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider hidden md:table-cell">Total Piutang</th>
                    <th class="px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider hidden md:table-cell">Pembayaran</th>
                    <th class="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider md:hidden">Aksi</th>
                </tr>
            </thead>
            ...
            .then(data => {
                ...
                data.forEach((item, index) => {
                    const row = `<tr>
                        ...
                        <td class="px-6 py-3 hidden md:table-cell">${item.kodepiutang}</td>
                        <td class="px-6 py-3 hidden md:table-cell">${item.jatuh_tempo}</td>
                        <td class="px-6 py-3 text-right hidden md:table-cell">${formatRupiah(item.total_piutang)}</td>
                        <td class="px-6 py-3 text-right hidden md:table-cell">${formatRupiah(item.total_pembayaran)}</td>
                        <td class="px-6 py-3 text-center md:hidden"><a href="/report-detail/${item.kodepiutang}" class="text-blue-500">Detail</a></td>
                    </tr>`;
