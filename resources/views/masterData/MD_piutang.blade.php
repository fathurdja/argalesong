                        ...
                        <ul class="space-y-1">
                            @foreach ($items as $item)
                                <li class="flex items-center">
                                    <span class="mr-2"><a href="{{ route('master_data_piutang_delete', ['id' => $item]) }}"
                                    onclick="return confirm('Apakah Anda yakin ingin menghapus?');"><svg xmlns="http://www.w3.org/2000/svg" class="text-red-500 hover:scale-110 active:scale-90" width="24" height="24" viewBox="0 0 24 24"><path fill="currentColor" d="M7 21q-.825 0-1.412-.587T5 19V6H4V4h5V3h6v1h5v2h-1v13q0 .825-.587 1.413T17 21zm2-4h2V8H9zm4 0h2V8h-2z"/></svg></a></span>
                                    {{ $item }}
                                </li>
                            @endforeach
                        ...
                        <h3 class="font-semibold mb-2 text-center bg-gray-100 py-1">{{ $title }}</h3>
                            <ul class="list-disc pl-6">
                                ...
                                <li class="flex items-center justify-between p-2 border-b">
                                    <span>{{ $item }}</span>
                                    <form action="{{ route('master_data_piutang_delete', ['id' => $loop->index]) }}" method="POST"
                                        onsubmit="return confirm('Apakah Anda yakin ingin menghapus?');">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="text-red-500 hover:scale-110 active:scale-90">
                        ...
                        <ul class="list-disc pl-6">
                            @foreach ($items as $item)
                                <li class="flex items-center justify-between p-2 border-b">
                                    ...
                                    <span>{{ $item }}</span>
                                    {{-- <form action="{{ route('master_data_piutang_delete', ['id' => $loop->index]) }}" method="POST"
                                        onsubmit="return confirm('Apakah Anda yakin ingin menghapus?');" class="inline">
                                      @csrf
                                      @method('DELETE')
                                      <button type="submit" class="text-red-500 hover:scale-110 active:scale-90">
                                      </button>
